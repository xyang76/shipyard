...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, durable bool) *Replica {
	...
	r.prepareRPC = r.RegisterRPC(new(Prepare), r.prepareChan)
	r.acceptRPC = r.RegisterRPC(new(Accept), r.acceptChan)
	r.commitRPC = r.RegisterRPC(new(Commit), r.commitChan)
	r.commitShortRPC = r.RegisterRPC(new(CommitShort), r.commitShortChan)
	r.prepareReplyRPC = r.RegisterRPC(new(PrepareReply), r.prepareReplyChan)
	r.acceptReplyRPC = r.RegisterRPC(new(AcceptReply), r.acceptReplyChan)
...
func (r *Replica) handleAccept(accept *Accept) {
	...
	if inst == nil {
		if accept.Ballot < r.defaultBallot {
			areply = &AcceptReply{accept.Instance, FALSE, r.defaultBallot}
		} else {
			r.instanceSpace[accept.Instance] = &Instance{
				...
				ACCEPTED,
				nil}
			areply = &AcceptReply{accept.Instance, TRUE, r.defaultBallot}
		}
	} else if inst.ballot > accept.Ballot {
		areply = &AcceptReply{accept.Instance, FALSE, inst.ballot}
	} else if inst.ballot < accept.Ballot {
		inst.cmds = accept.Command
		inst.ballot = accept.Ballot
		inst.status = ACCEPTED
		areply = &AcceptReply{accept.Instance, TRUE, inst.ballot}
