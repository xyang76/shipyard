...
type Replica struct {
	*genericsmr.Replica // extends a generic Paxos replica
	...
	prepareChan chan *gpaxosprotol.Prepare // channel for Prepare's

	m1aChan    chan *gpaxosprotol.M_1a
	m1bChan    chan *gpaxosprotol.M_1b
	m2aChan    chan *gpaxosprotol.M_2a
	m2bChan    chan *gpaxosprotol.M_2b
	commitChan chan *gpaxosprotol.Commit // channel for Commit's
...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool) *Replica {
	r := &Replica{genericsmr.NewReplica(id, peerAddrList, thrifty, exec, dreply),
		make(chan *gpaxosprotol.Prepare, CHAN_BUFFER_SIZE),
		make(chan *gpaxosprotol.M_1a, CHAN_BUFFER_SIZE),
		make(chan *gpaxosprotol.M_1b, CHAN_BUFFER_SIZE),
		make(chan *gpaxosprotol.M_2a, CHAN_BUFFER_SIZE),
		make(chan *gpaxosprotol.M_2b, CHAN_BUFFER_SIZE),
		make(chan *gpaxosprotol.Commit, CHAN_BUFFER_SIZE),
		false,
		0,
		3,
...
func (r *Replica) handleReplicaConnection(rid int, reader *bufio.Reader) error {
	...
	for !r.Shutdown && err == nil {
		...
		case gpaxosprotol.PREPARE:
			prep := new(gpaxosprotol.Prepare)
		...
		//case gpaxosprotol.M1A:
		//	msg := new(gpaxosprotol.M_1a)
		...
		case gpaxosprotol.M1B:
			msg := new(gpaxosprotol.M_1b)
			if err = msg.Unmarshal(reader); err != nil {
				break
			}
		...
		case gpaxosprotol.M2A:
			msg := new(gpaxosprotol.M_2a)
			if err = msg.Unmarshal(reader); err != nil {
				break
			}
			r.m2aChan <- msg
		...
		case gpaxosprotol.M2B:
			msg := new(gpaxosprotol.M_2b)
			if err = msg.Unmarshal(reader); err != nil {
				break
			}
			//HACK
		...
		case COMMIT:
			commit := new(gpaxosprotol.Commit)
...
func (r *Replica) replyPrepare(reply *gpaxosprotol.PrepareReply, w *bufio.Writer) {
	w.WriteByte(gpaxosprotol.PREPARE_REPLY)
	reply.Marshal(w)
	w.Flush()
}

func (r *Replica) send1b(msg *gpaxosprotol.M_1b, w *bufio.Writer) {
	w.WriteByte(gpaxosprotol.M1B)
	msg.Marshal(w)
...
func (r *Replica) send2b(msg *gpaxosprotol.M_2b, w *bufio.Writer) {
	w.WriteByte(gpaxosprotol.M2B)
	msg.Marshal(w)
	for _, cid := range msg.Cids {
		cmd := r.commands[cid]
		cmd.Marshal(w)
...
func (r *Replica) bcast2b(msg *gpaxosprotol.M_2b) {
	defer func() {
...
func (r *Replica) bcastPrepare(replica int32, instance int32, ballot int32) {
	...
	args := &gpaxosprotol.Prepare{r.Id, instance, ballot}
	...
	for sent := 0; sent < n; {
		...
		sent++
		w = r.PeerWriters[q]
		w.WriteByte(gpaxosprotol.PREPARE)
		args.Marshal(w)
		w.Flush()
	}
}
...
func (r *Replica) bcast1a(balnum int32, fast bool) {
	...
	args := &gpaxosprotol.M_1a{r.Id, balnum, TRUE}
	if !fast {
		args.Fast = FALSE
	}
...
func (r *Replica) bcast2a(balnum int32, cstruct []int32, fast bool) {
	...
	args := &gpaxosprotol.M_2a{r.Id, balnum, cstruct}
...
func (r *Replica) bcastCommit(cstruct []int32) {
	...
	args := &gpaxosprotol.Commit{cstruct}
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	...
	if r.isLeader {
		if !r.fastRound {
			...
			if ALL_TO_ALL {
				r.bcast2b(&gpaxosprotol.M_2b{r.Id, r.crtBalnum, crtBallot.cstruct, b[:1]})
			}
			r.tryToLearn()
		}
	} else {
		if r.fastRound {
			if crtBallot.received2a {
				if ALL_TO_ALL {
					...
					r.send2b(&gpaxosprotol.M_2b{r.Id, r.crtBalnum, crtBallot.cstruct, b[:1]}, r.PeerWriters[r.leaderId])
				}
			}
		}
	}
...
func (r *Replica) handlePrepare(prep *gpaxosprotol.Prepare) {
}

func (r *Replica) handle1a(msg *gpaxosprotol.M_1a) {
	...
	if r.crtBalnum >= 0 {
		r.send1b(&gpaxosprotol.M_1b{r.Id, msg.Balnum, r.ballotArray[r.crtBalnum].cstruct}, r.PeerWriters[r.leaderId])
	} else {
		r.send1b(&gpaxosprotol.M_1b{r.Id, msg.Balnum, make([]int32, 0)}, r.PeerWriters[r.leaderId])
	}
...
func (r *Replica) handle1b(msg *gpaxosprotol.M_1b) {
	if msg.Balnum != r.crtBalnum {
	...
	if (r.fastRound && count == r.fastQSize) ||
		...
		if r.fastRound && ALL_TO_ALL {
			r.bcast2b(&gpaxosprotol.M_2b{r.Id, r.crtBalnum, crtbal.cstruct, crtbal.cstruct})
		}
	}
}

func (r *Replica) handle2a(msg *gpaxosprotol.M_2a) {
	if r.isLeader {
	...
	if ALL_TO_ALL {
		r.bcast2b(&gpaxosprotol.M_2b{r.Id, r.crtBalnum, crtbal.cstruct, cids})
		r.tryToLearn()
	} else {
		r.send2b(&gpaxosprotol.M_2b{r.Id, r.crtBalnum, crtbal.cstruct, cids}, r.PeerWriters[r.leaderId])
	}
}

func (r *Replica) handle2b(msg *gpaxosprotol.M_2b) {
	if msg.Balnum != r.crtBalnum {
