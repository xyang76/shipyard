...
func Start() {
	...
	master := &Master{
		*config.NumNodes,
		make([]string, 0, *config.NumNodes),
		make([]string, 0, *config.NumNodes),
		make([]int, 0, *config.NumNodes),
		new(sync.Mutex),
		make([]*rpc.Client, *config.NumNodes),
		make([]bool, *config.NumNodes),
		make([]bool, *config.NumNodes),
	}

	rpc.Register(master)
	rpc.HandleHTTP()
	l, err := net.Listen("tcp", fmt.Sprintf(":%d", *config.MasterPort))
	if err != nil {
